#ifndef _PUBLIC_COR_SCH_H_
#define _PUBLIC_COR_SCH_H_
#include <inttypes.h>
#include <stddef.h>
#include <sys/epoll.h>
#include <sys/poll.h>
...
extern pthread_key_t global_sched_key; // 声明线程本地变量
extern pthread_once_t sched_key_once; // 在线程中只执行一次

struct cpu_ctx {
    void *rsp; // 栈顶指针
    void *rbp; // callee-saved register，用作数据存储
    void *rip; // 下一条指令
    void *rbx; // callee-saved register
    void *r12; // callee-saved register
    void *r13; // callee-saved register
    void *r14; // callee-saved register
    void *r15; // callee-saved register
};
...
enum CorStatus {
    ...
    STATUS_DETACH,
    STATUS_NEW,
    STATUS_FDEOF // file descriptor EOF
};
...
Coroutine::Coroutine(Coroutine **new_co, Entry &&f, Args &&... args) {
    // 保证多个线程只会执行一次初始化操作，用于初始化调度器（Schedule）的关键字（sched_key_once）
    ...
    // 一个 POSIX 标准中定义的内存分配函数，用于在指定的内存地址上分配内存块，并满足指定的对齐要求
