...
void Schedule::schedule_init_key()
{
    ...
    pthread_setspecific(global_sched_key, NULL); // 一个线程只能有一个调度器
}
...
Schedule *Schedule::get_schedule(void)
{ // 取出线程特有的值，即调度器
...
void Schedule::add_sleep(Coroutine *co, uint64_t msecs)
{
    ...
    if (co_tmp != co->sche->sleepSet.end())
    { // 如果找到了，需要删除后重新设置插入新的协程
...
void Schedule::run()
{
    while (!sleepSet.empty() || !ready.empty() || !wait.empty() || !exitQueue.empty())
    {
        ...
        while (num_new_events > 0)
        {
            ...
            if (is_eof)
                errno = ECONNRESET; // 如果对端关闭，设置errno
...
int Schedule::schedule_epoll(Schedule *sched)
{
    ...
    if (usecs <= 0 || !sched->ready.empty())
    { // 如果超时时间大于0，并且就绪队列为空,就继续执行
        return 0;
    }
