...
int poll_inner(pollfd *fds, nfds_t num_fd, int timeout)
{
	...
	// 注册完事件加入等待队列后，删除注册的事件，即定时任务结束删除此任务
	for (int i = 0; i < num_fd; ++i)
	{
...
int accept(int fd, struct sockaddr *addr, socklen_t *len)
{
	...
	while (1)
	{
		...
		fds.fd = fd;
		fds.events = POLLIN | POLLERR | POLLHUP;
		poll_inner(&fds, 1, timeout); // 把等待过程抽象为定时任务

		sockfd = accept_f(fd, addr, len); // 这里定时任务已经执行结束
		if (sockfd < 0)
		{
			if (errno == EAGAIN)
			{
...
int connect(int fd, struct sockaddr *name, socklen_t namelen)
{
	...
	while (1)
	{
		...
		fds.fd = fd;
		fds.events = POLLOUT | POLLERR | POLLHUP;
		poll_inner(&fds, 1, 1); // 等待定时任务做完

		ret = connect_f(fd, name, namelen); // 这里定时任务回来了，可以向下执行了
		if (ret == 0)
			break;
...
ssize_t recv(int fd, void *buf, size_t len, int flags)
{
	...
	int ret = recv_f(fd, buf, len, flags); // 这里定时任务回来了，可以向下执行了
	if (ret < 0)
	{
		// if (errno == EAGAIN) return ret;
		if (errno == ECONNRESET)
